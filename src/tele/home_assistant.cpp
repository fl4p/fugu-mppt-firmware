...
void haMqttBegin() {
    ...
    powerSensor = new HAMqttDevice(id.c_str(),
                                   HAMqttDevice::SENSOR,
                                   "homeassistant"
    );
    // https://www.home-assistant.io/integrations/sensor.mqtt/
...
void haMqttUpdate(const HAMqttFields &fields) {
    ...
    if (std::isfinite( fields.power)) {
        ...
        if (len < 1)
            ...
            len = esp_mqtt_client_publish(MQTT.client, powerSensor->getStateTopic().c_str(), buf, len, 0, 0);
            if (len < 0)
                ESP_LOGE("mqtt", "publish error %d", len);
            else
                ESP_LOGD("mqtt", "published %s: %s", powerSensor->getStateTopic().c_str(), buf);
        }
    }
}
