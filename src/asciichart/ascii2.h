...
namespace ascii {
    ...
    struct TwoDimSpan {
        ...
        TwoDimSpan(size_t n0, size_t n1, const T &v) : n0(n0), n1(n1) {
            // n0: rows, n1: cols
            //printf("alloc %u*%u*%u = %u\n", n0,n1, sizeof(T), n0*n1*sizeof(T));
            ptr = new Text[n0 * n1]{v};
            for(auto i = 0; i < (n0*n1); ++i)
                ptr[i] = v;
        }

        TwoDimSpan() : n0(0), n1(0) { ptr = nullptr; }
        ...
        T *operator[](const I &i0) {
            if(0 <= i0 and i0 < n0)
                ...
                throw std::range_error("index out of range " + std::to_string(i0));
        }
    };
    ...
    private:
        ...
        TwoDimSpan<Text> PlotLineChart() {
            ...
            int offset = offset_ + legend_cols;
            ...
            //printf("Free heap: %d, rows=%d cols=%d", heap_caps_get_free_size(MALLOC_CAP_8BIT), rows, cols);

            // 5. initialize chart using empty str
            //std::vector<std::vector<Text>> screen(
            //        rows + 1, std::vector<Text>(cols, symbols_["empty"]));

            auto screen = TwoDimSpan<Text>(rows + 1, cols, Text{" "});
          ...
          PutString(screen, label_trace_pair.first, style, j++, 0);
            ...
            // 8. Content
            {
                ...
                for (auto &label_trace_pair: series_) {
                    ...
                    auto style = styles_[j++ % styles_.size()];
                    int y0 = std::round(trace[0] * ratio) - min2;
                    // vertical reverse
                    screen[rows - y0][offset - 1] = Text(symbols_["center"], style);

                    for (size_t i = 0; i < trace.size() - 1; i++) {
                        int y0 = std::round(trace[i] * ratio) - min2;
                        int y1 = std::round(trace[i + 1] * ratio) - min2;
