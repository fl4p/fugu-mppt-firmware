...
void otaWatchdog(void *arg) {
    ESP_LOGI("ota", "Watchdog task started");
    auto tStart = wallClockUs();
    while (true) {
        vTaskDelay(pdMS_TO_TICKS(1000));
        if (wallClockUs() - tStart > OTA_TIMEOUT_SECONDS * int(1e6)) {
            ESP_LOGE("ota", "Ota timeout after %.1f seconds", (wallClockUs() - tStart) * 1e-6f);
            systemRestart();
        }
    }
    //ESP_LOGI("ota", "Watchdog task ends");
}
...
void doOta(const char *url) {
    ...
    esp_https_ota_config_t ota_config = {
        ...
        .partition{
            /*!< Details of staging and final partitions for OTA update */
            .staging = nullptr,
            /*!< New image will be downloaded in this staging partition. If NULL then a free app partition (passive app partition) is selected as the staging partition. */
            .final = nullptr,
