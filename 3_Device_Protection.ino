...
void Device_Protection(){
  ...
  //FAULT DETECTION     
  ERR = 0;                                                                                          //Reset local error counter                                                                             //Run backflow current protection protocol  
  ...
  if(output_Mode==0){   
    // PSU mode                                                                            //PSU MODE specific protection protocol
    REC = 0; BNC = 0;                                                                               //Clear recovery and battery not connected boolean identifiers
    if(voltageInput<voltageBatteryMax+voltageDropout){IUV=1;ERR++;errorCount++; Serial.println("!!! PSU IUV !!!"); }else{IUV=0;}        //IUV - INPUT UNDERVOLTAGE: Input voltage is below battery voltage (for psu mode only)                     
  }
  else{ 
