...
class EWM:
    ...
    @property
    def stddev(self, x=None):
        if x is not None:
            self.add(x)
        return abs(self.avg.value * self.std.value) ** .5

    #@property
    def nstddev(self, x=None):
        if x is not None:
            self.add(x)
        return abs(self.std.value) ** .5

    def z_score(self, x):
        return (x - self.avg.value) / (self.stddev + self.std_regularisation)

    def update_span(self, span):
        self.std.update_span(span)
        self.avg.update_span(span)
